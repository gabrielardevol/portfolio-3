  ...
  en: {
    translation: {
      ...
      projects: {
        ...
        pigs: {
          title: "🐺_🐷🐷🐷.rb",
          content: "Ruby on Rails was the technology with which I started programming. My learning style is very visual and results-oriented, and I enjoy engaging in creative exercises with what I learn. That's why I created this small video game inspired by the fable of the three little pigs, which runs in the CLI and is played using the spacebar on the keyboard. <br/>There are two levels: one where the pigs run away from the wolf through the forest, and another where the wolf huffs and puffs while the pigs try to keep the house from flying away. All of this is done using emojis as graphical elements to compose the different scenarios. It was fun!"
        }
      }
    }
  },
  ...
  esp: {
    translation: {
      ...
      projects: {
        ...
        esclat: {
          title: "Esclat EDC",
          content: "Fundació Esclat es una entidad sin ánimo de lucro que ofrece servicios a personas con parálisis cerebral. Tuve la oportunidad de colaborar con ellos creando una sección de su sitio web dedicada a la estrategia digital del centro, donde se publican ciertos aspectos de la gestión interna del grupo. <br/> Mi propuesta es una interfaz sencilla que respeta la identidad visual corporativa. Para darle mi toque personal, incluí algunas imágenes en las transiciones para que estuvieran presentes sin restar protagonismo a la documentación ni interferir innecesariamente en la legibilidad. Para lograrlo, presté especial atención al timing, como hacen los cineastas y animadores, haciendo que las transiciones fueran rápidas pero permitiendo apreciar las fotografías. Estas fotos aportan una dimensión emocional y hermosa a la documentación. <br/> Trabajar con Esclat fue genial porque buscaban un buen resultado, y yo también. Me proporcionaron muchas indicaciones para afinar el trabajo."


        },
        pigs: {
          title: "🐺_🐷🐷🐷.rb",
          content: "Ruby on Rails fue la tecnología con la que empecé a programar. Mi estilo de aprendizaje es muy visual y orientado a resultados, y me gusta realizar ejercicios creativos con lo que aprendo. Por eso creé este pequeño videojuego inspirado en la fábula de los tres cerditos, que se ejecuta en la interfaz de línea de comandos y se juega utilizando la barra espaciadora del teclado. <br/> Hay dos niveles: uno en el que los cerditos huyen del lobo a través del bosque, y otro en el que el lobo sopla mientras los cerditos intentan que la casa no salga volando. Todo esto se realiza utilizando emojis como elementos gráficos para componer los diferentes escenarios. ¡Fue divertido!"
        }
      }
    },
  },
  ja: {
    translation: {
      ...
      projects: {
        ...
        esclat: {
          title: "Esclat EDC",
          content: "Fundació Esclat、脳性麻痺を持つ人々にサービスを提供する非営利団体です。私は彼らと協力し、彼らのウェブサイトの一部を作成しました。この部分は、センターのデジタル戦略に関するもので、グループの内部管理の一部が公開されています。 <br/> 私の提案は、企業のビジュアルアイデンティティを尊重したシンプルなインターフェースです。個人的なアクセントを加えるために、トランジションにいくつかの画像を含め、文書の主役を奪わず、読みやすさに不必要な干渉を防ぎました。これを実現するために、映画製作者やアニメーターが行うように、タイミングに特別な注意を払い、トランジションを速く、同時に写真を鑑賞できるようにしました。これらの写真は文書に感情的で美しい次元をもたらします。 <br/> エスクラットと一緒に働くことは素晴らしかったです。彼らは良い結果を求めており、私も同じでした。彼らは作業を洗練させるための多くの指示を提供してくれました。"
        },
        pigs: {
          title: "🐺_🐷🐷🐷.rb",
